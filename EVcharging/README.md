...
For Rinkeby, you will need to connect your Metamask to Rinkeby network, and have some Ether in the Metamask account that can be obtained from  https://faucet.rinkeby.io/  Also, because of some ipfs artifacts, it takes slightly longer to load. On Rinkeby you can only Search for Charging Slots and Book and Pay for them. You will need to input use one of three cities Mumbai, Bangkok or Hong Kong, Delhi. You will also need to input DC or Level2 in the TYPE field. Please select time slot by entering Start time and End time in local format. On Rinkeby network (https://kejitan.github.io/EVcharging/) you will not have Admin or EVutility roles. 

The UI is available here:
  
* IPFS directly: Takes long time to load and has not been tried

You can test full functionality in the development mode, please see the *Play around* block in the [How to run it](#how-to-run-it) section to get an idea of what you can do.
...
* If you need to 'npm install libp2p-crypto', please do so.
* Ubuntu 16.04.5 
...
With an EV utility owner's account, you can go to "My EVutility" and see the EV stations that have been paid for by customers. You can see those ones you just bought, but make sure you log in with the proper EV utility. Also check your ETH balance, you'll have the revenue from booking the EV stations.
...
Feel free to play around with EV utilities and their owners in the admin panel, as well as with the EV stations of your EV utilities in the EVutilities panel. You may add more EV stations in exiting or new cities and then try finding and buying them in the "Search Charging Slots" tab.

Every EV utility has a logo, which is stored in IPFS. Sample data has logos uploaded from the `sample-images` directory. You can try updating logos of existing EV utilites or setting logos to new EV utilities that you can add from the Admin Panel..
...
The contracts deployed in Rinkeby testnet were verified by entering contract address (in deployed_addresses.txt) and ABI code to myetherwallet.com/#contracts

Smart Check was used to test the contracts
The contract files were uploaded to https://tool.smartdec.net/. Some issues are present but they are not severe.
